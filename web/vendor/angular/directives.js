...
app.directive("ngRenderFinish", ["$timeout", function ($timeout) {
    return {
        restrict: "A",
...
app.directive('tmPagination', [function () {
    return {
        ...
        '<div class="col-lg-6">'+
            ...
            '</ul> </div>' +
            '<div class="page-total col-lg-6" ng-show="conf.totalItems > 0">' +
            '<span>第</span><input class="page_box form-control" type="text" ng-model="jumpPageNum"  ng-keyup="jumpToPage($event)"/> <span>页 &nbsp;&nbsp;&nbsp;</span> ' +
            '<span>每页</span><select class="page_box form-control" ng-model="conf.itemsPerPage" ng-options="option for option in conf.perPageOptions " ng-change="changeItemsPerPage()"></select>' +
            '<span>/共<strong>{{ conf.totalItems }}</strong>条</span>' +
            '</div>' +
            '<div class="no-items" ng-show="conf.totalItems <= 0"><span>暂无数据</span></div>' +
            '</div>',
        replace: true,
        scope: {
            conf: '='
